...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: diskord.Member):
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[diskord.TextChannel, diskord.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`diskord.TextChannel` converter or a :class:`diskord.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`diskord.TextChannel`, and if it fails it tries to convert it to a :class:`diskord.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[diskord.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[diskord.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`diskord.Member` followed by a
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[diskord.Member] = commands.flag(name='member', default=lambda ctx: [])
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: diskord.Member):
